...
function Bone({ className }: { className?: string }) {
  return (
    <div
      className={cn(
        'bg-og-mid/50 rounded-lg animate-pulse',
        className,
      )}
    />
  );
    ...
    <div className="space-y-6">
      <div className="flex flex-col items-center py-4">
        <div className="flex items-center gap-3">
          <div className="w-5 h-5 border-2 border-og-primary border-t-transparent rounded-full animate-spin" />
      ...
      <div className="rounded-xl border border-og-mid/50 og-card p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="rounded-xl border border-og-mid/50 og-card p-4">
