...
const signalConfig = {
  bullish: { icon: TrendingUp, color: 'text-og-success', bg: 'bg-og-success/10 border-og-success/20', label: 'Bullish' },
  bearish: { icon: TrendingDown, color: 'text-og-error', bg: 'bg-og-error/10 border-og-error/20', label: 'Bearish' },
  neutral: { icon: Minus, color: 'text-zinc-400', bg: 'bg-og-mid/20 border-og-mid/30', label: 'Neutral' },
};
...
export function IndicatorCard({ indicator }: { indicator: IndicatorResult }) {
  ...
  return (
    <div className={cn('rounded-xl border p-4 transition-all hover:scale-[1.02] og-glow', config.bg)}>
